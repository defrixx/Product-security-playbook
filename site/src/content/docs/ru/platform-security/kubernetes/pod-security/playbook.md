...
- Минимальное runtime подтверждения: Linux `6.3+` или vendor-supported kernel с поддержкой ID-mapped mounts для всех файловых систем, которые использует workload, containerd `2.0+` или CRI-O `1.25+`, OCI runtime с поддержкой user namespaces, например `runc` `1.2+` или `crun` `1.9+`, и kubelet events/metrics, показывающие успешное создание user-namespace Pod.
- Rollout выполняйте поэтапно: сначала stateless рабочие нагрузки без `hostNetwork`, `hostPID`, `hostIPC`, raw block `volumeDevices` и специальных storage assumptions; затем stateful/storage-heavy рабочие нагрузки только после CSI/storage compatibility test; затем platform рабочие нагрузки по отдельному архитектурное ревью.
- Для images, которым реально нужен поведение, аналогичное root внутри контейнера, используйте отдельный exception path: владелец, expiry, причина несовместимости с `runAsNonRoot`, подтверждение, что host UID/GID остаются непривилегированными, и компенсирующие меры (`seccomp`, dropped capabilities, read-only root filesystem, restricted volumes).
