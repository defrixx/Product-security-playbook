...
- Capabilities при `hostUsers: false` становятся namespaced: например, `CAP_NET_ADMIN` может давать административные действия над локальными ресурсами контейнера, но не над host. Даже в таком режиме capabilities выдавайте только по явному обоснованию, с owner и expiry.
...
- Writable paths должны быть явными и ограниченными: `/tmp`, cache или каталоги логов выносите в отдельные mounts с понятным назначением, lifecycle и лимитами, а не оставляйте весь root filesystem доступным для записи.
...
- запрещайте runtime socket mounts через admission policy; исключения допускайте только для доверенных platform/build workload'ов с owner, expiry и компенсирующими контролями;
- после remediation повторите тот же query или policy test, чтобы подтвердить, что unsafe mount больше не проходит deploy.
...
- Adversarial validation для проверки pod-level abuse paths: [kubernetes/adversarial-validation/playbook.ru.md](../adversarial-validation/playbook.ru.md)
